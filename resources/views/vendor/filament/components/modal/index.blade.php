...
@php
    ...
    $hasFooter =
        !\Filament\Support\is_slot_empty($footer) ||
        (is_array($footerActions) && count($footerActions)) ||
        (!is_array($footerActions) && !\Filament\Support\is_slot_empty($footerActions));
    ...
    if (!$footerActionsAlignment instanceof Alignment) {
        $footerActionsAlignment = filled($footerActionsAlignment)
            ? Alignment::tryFrom($footerActionsAlignment) ?? $footerActionsAlignment
            : null;
    }
    ...
    $closeEventHandler = filled($id)
        ? '$dispatch(' .
            \Illuminate\Support\Js::from($closeEventName) .
            ', { id: ' .
            \Illuminate\Support\Js::from($id) .
            ' })'
        : 'close()';
@endphp
...
<div @if ($ariaLabelledby) aria-labelledby="{{ $ariaLabelledby }}"
    ...
    aria-modal="true" role="dialog" x-data="{
        isOpen: false,
    ...
    >
    @if ($trigger)
        <div x-on:click="open" {{ $trigger->attributes->class(['fi-modal-trigger flex cursor-pointer']) }}>
            {{ $trigger }}
        </div>
    @endif

    <div x-cloak x-show="isOpen">
        <div aria-hidden="true" x-show="isOpen" x-transition.duration.300ms.opacity @class([
        ...
        <div @class([
            ...
            <div x-ref="modalContainer"
                @if ($closeByClickingAway) {{-- Ensure that the click element is not triggered from a user selecting text inside an input. --}}
                    x-on:click.self="
                ...
                <div x-data="{ isShown: false }" x-init="$nextTick(() => {
                    ...
                    {{ ($extraModalWindowAttributeBag ?? new \Illuminate\View\ComponentAttributeBag())->class([
                        ...
                        match ($width) {
                            ...
                            MaxWidth::FourExtraLarge => 'max-w-7xl',
                    ...
                    @if ($heading || $header)
                        <div @class([
                            ...
                            'rounded-t-xl' =>
                                $stickyHeader && !($slideOver || $width === MaxWidth::Screen),
                            match ($alignment) {
                            ...
                            @if ($closeButton)
                                <div @class([
                                    ...
                                    <x-filament::icon-button color="gray" icon="heroicon-o-x-mark"
                                        ...
                                        :x-on:click="$closeEventHandler" class="fi-modal-close-btn" />
                                </div>
                            @endif
                                ...
                                @if ($hasIcon)
                                    <div @class([
                                        ...
                                        <div @class([
                                            ...
                                            \Filament\Support\get_color_css_variables($iconColor, shades: [100, 400, 500, 600], alias: 'modal.icon') => $iconColor !== 'gray',
                                        ])>
                                            <x-filament::icon :alias="$iconAlias" :icon="$icon"
                                                @class([
                    ...
                    @if ($hasSlot)
                        <div @class([
                            ...
                            'pe-6 ps-[5.25rem]' =>
                                $hasIcon && $alignment === Alignment::Start && !$stickyHeader,
                    ...
                    @if ($hasFooter)
                        <div @class([
                            ...
                            'rounded-b-xl' =>
                                $stickyFooter && !($slideOver || $width === MaxWidth::Screen),
                            'pb-6' => !$stickyFooter,
