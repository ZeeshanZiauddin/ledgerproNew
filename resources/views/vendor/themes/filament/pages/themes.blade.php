        ...
        <div class="flex items-center gap-4 border-t py-6">
            @if ($this->getCurrentTheme() instanceof \Hasnayeen\Themes\Contracts\HasChangeableColor)
                @foreach ($this->getColors() as $name => $color)
                    <button wire:click="setColor('{{ $name }}')" @class([
    ...
    <section class="">
        ...
        <div class="grid grid-cols-1 gap-6 border-t py-6">
            @foreach ($this->getThemes() as $name => $theme)
                @php
                    $noLightMode = in_array(
                        \Hasnayeen\Themes\Contracts\HasOnlyDarkMode::class,
                        class_implements($theme),
                    );
                    $noDarkMode = in_array(
                        \Hasnayeen\Themes\Contracts\HasOnlyLightMode::class,
                        class_implements($theme),
                    );
                    $supportColorChange = in_array(
                        \Hasnayeen\Themes\Contracts\HasChangeableColor::class,
                        class_implements($theme),
                    );
                @endphp
                ...
                <x-filament::section>
                            ...
                            @if ($supportColorChange)
                                <span x-data="{}"
                                    ...
                                    <svg class="w-4 h-4 dark:text-gray-800" viewBox="0 0 24 24" fill="none"
                                        ...
                                        stroke-linejoin="round" class="lucide lucide-paintbrush-2">
                                        <path
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            @if ($noLightMode)
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">
                                    {{ __('themes::themes.no_light_mode') }}</h3>
                            @else
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">{{ __('themes::themes.light') }}
                                </h3>
                                <img src="{{ url('https://raw.githubusercontent.com/Hasnayeen/themes/3.x/assets/' . $name . '-light.png') }}"
                        ...
                        <div>
                            @if ($noDarkMode)
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">
                                    {{ __('themes::themes.no_dark_mode') }}</h3>
                            @else
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">{{ __('themes::themes.dark') }}
                                </h3>
                                <img src="{{ url('https://raw.githubusercontent.com/Hasnayeen/themes/3.x/assets/' . $name . '-dark.png') }}"
